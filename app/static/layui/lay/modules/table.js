...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util', 'laydate'], function(exports) {
    ...
    Class.prototype.events = function() {
        ...
        that.layBody.on('click', 'td', function() {
            ...
            //////20180829修改
            if (editType) {
                if (editType === 'number') { //数字
                    input = $('<input type="number" min="0" class="layui-input ' + ELEM_EDIT + '" blur>');
                    input[0].value = othis.data('content') || elemCell.text();
                    othis.find('.' + ELEM_EDIT)[0] || othis.append(input);
                    input.focus();
                    return;
                }else if (editType === 'images') { //查看图片
                    ...
                    input = $('<input class="layui-input ' + ELEM_EDIT + ' calendar"  readonly >');
                    input[0].value = othis.data('content') || elemCell.text();
                    othis.find('.' + ELEM_EDIT)[0] || othis.append(input);
                    input.focus();
                        ...
                        laydate.render({
                            ...
                            trigger: 'click',
                                ...
                                var index = othis.parents('tr').eq(0).data('index'),
                                    data = table.cache[that.key][index];
                                data[field] = value; //更新缓存中的值
                                // othis 单元格属性绑定到commonMember
                                layui.event.call(this, MOD_NAME, 'edit(' + filter + ')', commonMember.call(othis, {
                                    value: value,
                                    field: field,
                                }));
                            }
                    ...
                    var value = othis.data('content') || elemCell.text();
                    layer.prompt({
                        formType: 2,
                        title: false,
                        value: value,
        ...
        //单元格展开事件
        that.layBody.on('click', '.' + ELEM_GRID_DOWN, function(event) {
          //20180829修改，阻止单元格展开时间出发单元格点击事件
